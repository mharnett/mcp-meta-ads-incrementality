...
# fake-security-on-PATH technique.
set -uo pipefail
...
echo "case: token present -> resolved and node launched"
run_case "meta-ads-mcp|META_ACCESS_TOKEN|tok123"
assert_contains "$OUT" "TOKEN=tok123"
assert_rc "$RC" 0

echo "case: token missing -> fatal, exit 1"
run_case ""
assert_contains "$OUT" "[FATAL] META_ACCESS_TOKEN is empty"
assert_rc "$RC" 1
